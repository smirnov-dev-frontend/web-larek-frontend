...
**Основной метод класса**, предназначенный для отображения данных в интерфейсе.
...
`handleOverlayClick(event: MouseEvent): void` - vетод-обработчик клика по фону (оверлею) модального окна.
