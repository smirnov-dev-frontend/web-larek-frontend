...
- хранит ссылки на используемые DOM-элементы, колбэки и экземпляр `EventEmiter`;
...
`isInCart: boolean` — флаг UI-состояния кнопки добавления в корзину (используется только для отображения текста кнопки "В корзине"/"В корзину" и визуального выделения карточки, не является источником данных о состоянии корзины и перезаписывается при каждом рендере новыми данными из презентера).
##### Методы класса:
...
- полностью очищает контейнер корзины;
- добавляет все переданные DOM-элементы товаров в `itemsContainer`;
- обновляет отображение итоговой стоимости в `totalElement`;
- управляет состоянием кнопки оформления заказа (включает при наличии товаров, отключает при пустой корзине);
- возвращает корневой элемент `container`.
...
- `contactsView: IOrderContactsView` — отображение формы контактных данных;
- `addressView: IOrderAddressView` — отображение формы адреса доставки и оплаты;
- `successView: IOrderSuccessView` — отображение экрана успешного оформления;
- `cartModel: ICartModel` — модель корзины;
- `events: EventEmitter` — брокер событий приложения.
##### Поля класса:
- `orderModel` — ссылка на модель заказа;
- `contactsView` — ссылка на отображение формы контактных данных;
- `addressView` — ссылка на отображение формы адреса и оплаты;
- `successView` — ссылка на отображение экрана успеха;
- `cartModel` — ссылка на модель корзины;
- `events` — брокер событий приложения;
- `orderData: Partial<ApiOrderRequest>` — временное хранилище данных заказа.
##### Методы класса
`init(): void`
- подписывается на события форм оформления заказа:
  - `order:contacts:submit` — отправка контактных данных;
  - `order:address:submit` — отправка адреса и способа оплаты;
  - `order:success:close` — закрытие экрана успешного оформления;
- управляет переходами между шагами оформления;
- контролирует сценарий отправки заказа.

`handleContactsSubmit(data: { email: string; phone: string }): void`
- сохраняет контактные данные во временное хранилище;
- скрывает форму контактных данных;
- отображает форму адреса и оплаты.

`handleAddressSubmit(data: { address: string; payment: 'card' | 'cash' }): void`
- сохраняет данные адреса и оплаты во временное хранилище;
- формирует полный объект заказа, объединяя:
  - сохранённые контактные данные;
  - данные адреса и оплаты;
...
- скрывает форму адреса и оплаты;
- передаёт итоговую сумму заказа в `successView`;
- отображает экран успешного оформления;
- очищает корзину через `CartModel`.

`reset(): void`
- очищает временное хранилище данных заказа;
- скрывает все формы оформления;
- используется при завершении оформления или отмене.
