...
export function cloneTemplate<T extends HTMLElement>(query: string | HTMLTemplateElement): T {
    const template =
        typeof query === 'string' ? ensureElement<HTMLTemplateElement>(query) : query;

    if (!template.content.firstElementChild) {
        throw new Error(`Template ${typeof query === 'string' ? query : 'template'} has no content`);
    }
...
export function setElementData<T extends Record<string, unknown> | object>(el: HTMLElement, data: T): void {
    for (const key in data as Record<string, unknown>) {
        el.dataset[key] = String((data as Record<string, unknown>)[key]);
    }
}
...
export function getElementData<T extends Record<string, unknown>>(
    ...
    for (const key in el.dataset) {
        const parser = scheme[key];
        if (!parser) continue;
        data[key as keyof T] = parser(el.dataset[key]) as T[keyof T];
    }
    return data as T;
}
